---
title: "check coverage after adding weights, with estimated correlation pairs"
author: "DongyueXie"
date: "2021-07-25"
...
simu_study = function(ref,
  ...
  if(!est_cor){
    ...
    X_array_bulk_for_cor = array(dim=c(G,K,n_bulk_for_cor))
    for(k in 1:K){
      if(is.indep){
        X_array_bulk_for_cor[,k,] = exp(matrix(rnorm(G*n_bulk_for_cor,n.ref[,k],sqrt(n.Sigma[,k])),ncol=n_bulk_for_cor))
      }else{
        X_array_bulk_for_cor[,k,] = t(exp(mvnfast::rmvn(n_bulk_for_cor,mu = n.ref[,k],sigma = n.Sigma.chol[[k]],isChol = TRUE)))
      }
    }
...
summary_simu_study = function(out1){
  ...
  boxplot(out1$p_hat_se,col='grey80',ylab='se',ylim = range(apply(out1$p_hat_se,2,quantile,c(0,0.99),na.rm=T)))
  lines(apply(out1$p_hat, 2, sd,na.rm=TRUE),type='p',col='yellow',pch=17)
