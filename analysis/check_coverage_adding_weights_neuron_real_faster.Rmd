...
A: positive in more than $25\%$ individuals, 383886 pairs

```{r}
fit = readRDS('output/check_coverage_weight_neuron/25quantileA.rds')
summary_temp(fit,b,true.beta)
```

A: positive in more than $50\%$ individuals, 16472198 pairs

```{r}
fit = readRDS('output/check_coverage_weight_neuron/50quantileA.rds')
summary_temp(fit,b,true.beta)
```

A: positive in more than $75\%$ individuals, 25608838 pairs

```{r}
fit = readRDS('output/check_coverage_weight_neuron/75quantileA.rds')
summary_temp(fit,b,true.beta)
```

Take mean of residual first then get A

```{r}
fit = readRDS('output/check_coverage_weight_neuron/mean_res_A.rds')
summary_temp(fit,b,true.beta)
```
