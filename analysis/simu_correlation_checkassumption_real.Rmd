...
randomly subsample 1000 gene, 4 cell type, 4 bulk individuals
...
```{r}
par(mfrow=c(1,1))
plot(apply(simu_trueb$est_adj,2,sd),type='b',
     ylim=range(c(apply(simu_trueb$est_adj,2,sd),
                  apply(simu_trueb$se_adj_hc0,2,mean,na.rm=T),
                  apply(simu_trueb$se_unadj_hc0,2,mean,na.rm=T))))
lines(apply(simu_trueb$se_adj_hc0,2,mean,na.rm=T),type='b',col=3)
lines(apply(simu_trueb$se_unadj_hc0,2,mean,na.rm=T),type='b',col=4)
legend('bottomright',c('true p_hat se','estimated with cor','estimated without cor'),lty=c(1,1,1),pch=c(1,1,1),col=c(1,3,4))
```
...
n.Sigma = matrix(nrow=G,ncol=K)
  for(k in 1:K){
    ...
    n.s = sqrt(log(1+sigma2.sub[,k]/ref.sub[,k]^2))
