...
```{r}
par(mfrow=c(1,1))
plot(apply(simu$est_adj,2,sd),type='b',ylim=range(c(apply(simu$est_adj,2,sd),
                                                    apply(simu$se_adj_hc0,2,mean,na.rm=T),
                                                    apply(simu$se_unadj_hc0,2,mean,na.rm=T))))
...
Use true beta for calculating the variance
...
# saveRDS(simu,file='output/simu_correlation_checkavariance_betahat_d500_xin_trueb.rds')
simu = readRDS('output/simu_correlation_checkavariance_betahat_d500_xin_trueb.rds')
```
