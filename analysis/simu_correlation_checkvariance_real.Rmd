...
#                         verbose = F,
#                         printevery = 1)
# saveRDS(simu,file='output/simu_correlation_checkavariance_betahat_d50_xin.rds')
simu = readRDS('output/simu_correlation_checkavariance_betahat_d50_xin.rds')
```
...
The reason is probably: In the generation of $X$, it's mean is different from $U$ due to the large variance.
...
### filter out large variances

filter out large variances

```{r}
# remove sigma2 that's too large
rm.idx = apply(sigma2,2,function(z){which(z>=quantile(z,0.80))})
...
#                         verbose = F,
#                         printevery = 1)
# saveRDS(simu,file='output/simu_correlation_checkavariance_betahat_d50_xin_sigma2small.rds')
simu = readRDS('output/simu_correlation_checkavariance_betahat_d50_xin_sigma2small.rds')
```
...
```{r}
par(mfrow=c(1,1))
plot(apply(simu$est_adj,2,sd),type='b',ylim=c(0,0.16))
lines(apply(simu$se_adj_hc0,2,mean,na.rm=T),type='b',col=3)
legend('bottomright',c('true p_hat se','estimated'),lty=c(1,1),pch=c(1,1),col=c(1,3))
```


### filter out large variances, and then divide them by 2


```{r}
apply(sigma2/2,2,function(z){round(quantile(z),3)})
```
...
#                         verbose = F,
#                         printevery = 1)
# saveRDS(simu,file='output/simu_correlation_checkavariance_betahat_d50_xin_sigma2smaller.rds')
simu = readRDS('output/simu_correlation_checkavariance_betahat_d50_xin_sigma2smaller.rds')
```
...
#                         verbose = F,
#                         printevery = 1)
# saveRDS(simu,file='output/simu_correlation_checkavariance_betahat_d500_xin.rds')
 simu = readRDS('output/simu_correlation_checkavariance_betahat_d500_xin.rds')
```
...
#                         verbose = F,
#                         printevery = 1)
# saveRDS(simu,file='output/simu_correlation_checkavariance_betahat_d500_xin_sigma2small.rds')
simu = readRDS('output/simu_correlation_checkavariance_betahat_d500_xin_sigma2small.rds')
```
...
#                         verbose = F,
#                         printevery = 1)
# saveRDS(simu,file='output/simu_correlation_checkavariance_betahat_d500_xin_sigma2smaller.rds')
simu = readRDS('output/simu_correlation_checkavariance_betahat_d500_xin_sigma2smaller.rds')
```
