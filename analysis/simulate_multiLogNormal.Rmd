...
Let $\Sigma = VRV$ where $V$ is a diagonal matrix of standard deviations $V = diag(\sigma)$, $\sigma = \sqrt{diag(\Sigma)}$,  and $R$ is the correlation matrix.

First generate 

\[y\sim N(\log\frac{\mu^2}{\sqrt{\mu^2+\sigma^2}},V'RV'),\]

where $V' = diag\{\sqrt{\log(1+\frac{\sigma^2}{\mu^2})}\}$ and the square and log are taken elementwisely.

Let $x = \exp(y)$, then $E(x) = \mu$, $diag\{cov(x)\} = \sigma^2$.

The correlations won't match but mean and variance match.
