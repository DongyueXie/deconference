...
#main function
CIBERSORT <- function(X, Y, perm=0, QN=TRUE, absolute=FALSE, abs_method='sig.score'){
...
library(qvalue)
#'@param ref_samples a reference matrix, rows are genes with gene names, columns are cell types. Each cell type should have at least 3 replications.
#'
build_signature_matrix_CIBERSORT = function(ref_samples,q_level=0.3,G_range = c(50,200),QN=TRUE){
  G = nrow(ref_samples)
  genes = rownames(ref_samples)
  ref_cells = colnames(ref_samples)
  cell_types = unique(ref_cells)
  K = length(cell_types)
  if(QN){
    ref_samples = normalize.quantiles(as.matrix(ref_samples),copy = FALSE)
  }



  ref_mat = matrix(nrow=G,ncol=K)
  colnames(ref_mat) = cell_types
  rownames(ref_mat) = genes
  # find de genes for each cell types, and record their fold change
  de_genes = list()
  for(k in 1:K){
    cell.col.idx = which(ref_cells==cell_types[k])
    ref_mat[,k] = rowMeans(ref_samples[,cell.col.idx])
    p_value = c()
    fold_change = c()
    for(g in 1:G){
      ttest = try(t.test(ref_samples[g,cell.col.idx],ref_samples[g,-cell.col.idx],alternative = "two.sided", var.equal = FALSE))
      if(class(ttest)=="try-error"){
        p_value[g] = 1
        fold_change[g] = 0
      }else{
        p_value[g] = ttest$p.value
        fold_change[g] = as.numeric(ttest$estimate[1] - ttest$estimate[2])
      }
    }
  ...
  names(de_genes) = cell_types

  # select top genes from each cell types and find the combination with smallest kappa.


  current_gene_idx = c()
  for(k in 1:K){
    if(length(de_genes[[k]]$gene_idx)>=G_range[1]){
      current_gene_idx = c(current_gene_idx,de_genes[[k]]$gene_idx[1:G_range[1]])
    }else{
  ...
  for(g in ((G_range[1]+1):G_range[2])){

    add_gene_idx =c()
    for(k in 1:K){
      if(length(de_genes[[k]]$gene_idx)>=g){
        add_gene_idx = c(add_gene_idx,de_genes[[k]]$gene_idx[g])
      }
    }
    if(length(add_gene_idx)>0){
      current_gene_idx = c(current_gene_idx,add_gene_idx)
      current_gene_idx = unique(current_gene_idx)
    }else{
      break
    }

    new_ref_mat = ref_mat[current_gene_idx,]
    if(kappa(new_ref_mat)<kappa0){
      best_ref_mat = new_ref_mat
    }
